data {
	...
	int<lower=1, upper=n_users> users[N]; // clients
...
transformed parameters{
    vector<lower=0, upper=1>[C] phi; // according to Anh. et al 2009 
    phi[1] = 0;
    phi[2] = gamma[1];
...
model{
	...
	for(i in 1:N){
		mu[1] = exp(0 + phi[1] * (x[i] * beta[users[i]]));
		
		for(j in 2:C)
			mu[j] = exp(alphaIntercept[j-1] + phi[j] * (x[i] * beta[users[i]]));
...
generated quantities{
	...
	for(i in 1:N){
		GeneratedMu[1] = exp(0 + phi[1] * (x[i] * beta[users[i]]));
		
		for(j in 2:C)
			GeneratedMu[j] = exp(alphaIntercept[j-1] + phi[j] * (x[i] * beta[users[i]]));
	...
	for(i in 1:N2){
		mu_pred[1] = exp(0 + phi[1] * (x_pred[i] * beta[users2[i]]));
		
		for(j in 2:C)
			mu_pred[j] = exp(alphaIntercept[j-1] + phi[j] * (x_pred[i] * beta[users2[i]]));
